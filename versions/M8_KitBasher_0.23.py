...
# Version 0.23 - Stereo support added!
# You'll need tkinter, customtkinter, pydub, wave, and os.
# Written in October 2023 - Andy Tanguay.
# Updated November 2025 - Stereo fix implemented (samples vs frames)
# Note: Now preserves stereo! Cue points use frame positions, not sample positions.
...
def concatenate_audio_files(file_names, output_file, marker_duration_ms=1, silence_thresh=-50.0, min_silence_len=10, retained_silence=1, force_mono=False):
    """
    Concatenate audio files with M8-compatible cue markers.

    Args:
        file_names: List of WAV file paths
        output_file: Output file path
        marker_duration_ms: Silent marker duration between slices (default 1ms)
        silence_thresh: Silence threshold in dBFS (default -50.0)
        min_silence_len: Minimum silence length to detect in ms (default 10)
        retained_silence: Silence to keep between chunks in ms (default 1)
        force_mono: Force conversion to mono for compatibility (default False)
    """
    ...
    for i, file in enumerate(file_names):
        ...
        if i == 0:
            ...
            marker = AudioSegment.silent(duration=marker_duration_ms, frame_rate=audio.frame_rate)
            marker = marker.set_channels(target_channels)
            retain_silence = AudioSegment.silent(duration=retained_silence, frame_rate=audio.frame_rate)
            ...
            marker_frames = marker_samples // marker.channels  # Convert samples to frames
    ...
    try:
        ...
        channel_desc = f"{target_channels} channel{'s' if target_channels > 1 else ''} ({'stereo' if target_channels == 2 else 'mono' if target_channels == 1 else 'multi-channel'})"
    ...
    messagebox.showinfo("Success", f"Files have been merged successfully!\n\nOutput: {channel_desc}\nCue points: {len(cue_positions)}")
    return True
